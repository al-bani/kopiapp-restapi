...
async function register(req, res) {
  ...
  try {
    ...
    const emailExits = await prisma.customer.findFirst({
      where: { email: email },
    });

    const phoneExits = await prisma.customer.findFirst({
      where: { phone_number: req.body.phone_number },
    });

    if (phoneExits && emailExits) {
      ...
      bcrypt.compare(
        ...
        async (err, bResult) => {
          if (err) {
            ...
            const token = signJwt(result.customer_id);
            ...
            await prisma.customer.update({
              ...
              data: {
                last_login: dateNow.toISOString,
              },
            });
...
const getCustomerDetails = (req, res) => {
  ...
  let query = `SELECT * FROM customer WHERE customer_id = ${db.escape(
    customer_id
  )}`;
...
const updateCustomerDetails = (req, res) => {
  ...
  const query = `UPDATE customer SET name = ${db.escape(
    req.body.name
  )}, email = ${db.escape(req.body.email)}, phone_number = ${db.escape(
    req.body.phone_number
  )}, age = ${db.escape(req.body.age)} WHERE customer_id = ${db.escape(
    customer_id
  )}`;
  db.query(query, (err, result) => {
    if (err) {
    ...
    return res
      .status(200)
      .send({ msg: "Customer details updated successfully" });
  });
};
...
const updatePasswordCustomer = async (req, res) => {
  ...
  if (req.body.userType === "login") {
    ...
    const query = `SELECT customer_id FROM customer WHERE email = ${db.escape(
      req.body.signature
    )} OR phone_number = ${db.escape(req.body.signature)}`;
  ...
  db.query(
    `SELECT password FROM customer WHERE customer_id = ${db.escape(
      customer_id
    )}`,
    (err, result) => {
      if (err) {
        ...
        bcrypt.compare(
          ...
          (err, bResult) => {
            if (err) {
              ...
              return res.status(401).send({
                msg: "New password do not match with the old password",
              });
            } else {
              bcrypt.hash(req.body.new_password, 10, (err, hash) => {
                if (err) {
                  ...
                  const query = `UPDATE customer SET password = ${db.escape(
                    hash
...
const sendEmailOTP = async (req, res) => {
  ...
  const htmlTemplate = fs.readFileSync(
    path.join(__dirname, "/assets/email/verification.html"),
    "utf-8"
  );
...
const addRating = (req, res) => {
  ...
  const query = `INSERT INTO ratings (customer_id, ${typeColumn}, rating_value, description, date_created) VALUES (${db.escape(
    customer_id
  )}, ${db.escape(req.body.type_id)}, ${db.escape(
    req.body.rating_value
  )}, ${db.escape(req.body.description)}, ${db.escape(dateNow)})`;

  db.query(
    `SELECT * FROM ratings WHERE customer_id = ${db.escape(
      customer_id
    )} AND ${typeColumn} = ${db.escape(req.body.type_id)}`,
    (err, result) => {
      if (err) {
        return res.status(500).send({ msg: "Internal Server Error", err });
      }
...
function signJwt(customer_id) {
  ...
  const token = jwt.sign(
    ...
    { algorithm: "RS256", expiresIn: timeExpiredToken }
  );
...
function verifyTokenJWT(token, customer_id) {
  if (
    !token ||
    !token.startsWith(process.env.PASSKEY) ||
    !token.split(" ")[1]
  ) {
    return null;
  }
